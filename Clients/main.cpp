...
private:
    ...
    void on_read(const error_code & err, size_t bytes) {
        if ( !err) {
            ...
            std::cout<<mor.Code(message_)<<std::endl;
...
void Asio_Client(int diff_task)
{
    ...
    for (int i = 97; i < 122; i++)
        symb += char(i);
    symb +=' ';
    std::cout<<rand()%(symb.size()-1);
    for (int i = 0; i < diff_task; i++)
        message += symb[rand()%(symb.size()-1)];
    //for ( char ** message = messages; *message; ++message) {
        talk_to_svr::start(ep, message.c_str());
...
int main(int argc, char* argv[]) 
{
    if (argv[1][0] == '0')
    {
        Client cli(atoi(argv[2]));
        cli.run();
    }
    else
    {
        Asio_Client(atoi(argv[2]));
    }
